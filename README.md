...
Таким образом, SynthScore интерпретируется следующим образом:

•	0–4: очень “простые” цели (короткий путь, высокий  `best_state_score` , небольшой  `number_of_nodes` , быстрый поиск).

•	4–8: средняя сложность.

•	8–12: высокая сложность (много шагов и/или слабый  `best_state_score` , и/или тяжёлый поиск).

•	12–16: очень сложные, но всё ещё решаемые AiZynthFinder.

•	≥16: по определению это no-route (если такие появятся), и дальше ранжирование идёт внутри no-route за счёт  `SA_score` .
