...
Шкала: **чем выше SynthScore, тем сложнее синтез**.  
...
Дополнительно для ретросинтеза:
- AiZynthFinder
- Public data для AiZynthFinder (policy модели + stock). config.yml файл был немного модифицирован относительно дефолтно скачиваемого. Измененный мной вариант приложен в репозитории.
...
SynthScore =
  1.0 * max_transforms +
  3.0 * (1 - best_state_score) +
  0.5 * log10(1 + number_of_nodes) +
  0.5 * (search_time / 60)

  где 	•	 max_transforms : прокси числа стадий; больше стадий → сложнее.
...
Если  status != "solved"  или  n_routes == 0 , скоринг всё равно должен быть численным и различать такие молекулы (требование задания).
